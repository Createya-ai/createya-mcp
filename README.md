...
#### Через MCP или REST. Из России. Без VPN. За рубли.
...
[![Made in Russia](https://img.shields.io/badge/Made%20in-Russia-EF4444?style=flat-square)]()
...
**Подключи нейросети Createya к своему AI-агенту через MCP или к своему коду через REST. Работает прямо из России без VPN. Оплата в рублях. 100 кредитов бесплатно на старте.**
...
Но если ты в России — упираешься в стену:
- **Нет доступа** к зарубежным AI-сервисам без VPN и зарубежных карт
- **Платить нечем** — Stripe, PayPal, западные подписки не работают
- **Каждый сервис отдельно** — везде свой ключ и подписка
- **Регулирование 152-ФЗ** — для бизнеса нельзя гонять данные пользователей через зарубежные дата-центры

**Createya решает все 4 проблемы одним подключением:**

| Проблема | Решение Createya |
|---|---|
| Нет доступа из РФ | 🇷🇺 Серверы в Yandex Cloud, работает без VPN |
| Нечем платить | 💳 Карты РФ, СБП, Т-Пэй — оплата в рублях |
| Десятки сервисов | 🎨 Каталог моделей через **один API-ключ** |
| 152-ФЗ | 🔒 Все данные хранятся в РФ (Yandex Cloud), полное соответствие закону |
...
- **152-ФЗ** — все данные хранятся в РФ (Yandex Cloud), полное соответствие закону «О персональных данных»
...
- **152-ФЗ** — все данные хранятся в РФ (Yandex Cloud)
...
**Сделано в России 🇷🇺 для AI-агентов всего мира.**
